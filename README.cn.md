# triton-inference-server

[English version](./README.md)

The Triton Inference Server provides an optimized cloud and edge inferencing solution. 

[![x-cmd/install — triton-inference-server Code Quality Monitoring Repo Card](https://repo.x-cmd.io/triton-inference-server.svg?lang=zh)](https://x-cmd.com/install/triton-inference-server)

## 安装

```sh
x install triton-inference-server
```

## 代码洞察

合计: **141,504** 行代码（覆盖前 5 种语言、共 **672** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Python | 77,253 | 16,606 | 12,012 | 406 |
| Sh | 27,473 | 7,073 | 5,401 | 181 |
| Cpp | 22,147 | 2,993 | 3,049 | 32 |
| Json | 4,011 | 0 | 4 | 34 |
| CHeader | 3,619 | 1,368 | 672 | 19 |

## 源代码

- **上游仓库**: <https://github.com/NVIDIA/triton-inference-server>
- **官网**: <https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/index.html>
- **许可证**: BSD-3-Clause

## 发布

- **最新版本**: `v2.72.0` (2026-08-31)
- **最近提交**: 2026-09-17
- **Release 含资产**: 5 个

## 流行度

- **Star**: 11,003 · **Fork**: 1,837 · **开放 issue**: 4,165 · **贡献者**: 147

## 累计统计

- **发布数**: 94 · **已合并 PR**: 4034 · **开放 PR**: 116 · **已关闭 issue**: 3386 · **开放 issue**: 779 · **提交数**: 3859

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-23 | 1 | 0 | 0 | 0 | 0 | 0 |
| last60d | 2026-07-24 | 2 | 0 | 0 | 0 | 0 | 0 |
| 90d | 2026-06-24 | 3 | 0 | 0 | 0 | 0 | 0 |
| last180d | 2026-03-26 | 6 | 0 | 0 | 0 | 0 | 0 |
| 360d | 2025-09-27 | 12 | 0 | 0 | 0 | 0 | 0 |
| last720d | 2024-10-02 | 23 | 0 | 0 | 0 | 0 | 0 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [tritonserver-2.72.0+nv26.08-cu134-cp312-manylinux_2_34-aarch64.zip](https://github.com/NVIDIA/triton-inference-server/releases/download/v2.72.0/tritonserver-2.72.0+nv26.08-cu134-cp312-manylinux_2_34-aarch64.zip) | 958.6 MiB | `native/linux/arm64` |
| [tritonserver-2.72.0+nv26.08-cu134-cp312-manylinux_2_34-x86_64.zip](https://github.com/NVIDIA/triton-inference-server/releases/download/v2.72.0/tritonserver-2.72.0+nv26.08-cu134-cp312-manylinux_2_34-x86_64.zip) | 988.9 MiB | `native/linux/x64` |
| [tritonserver_sdk-2.72.0+nv26.08-cu134-cp312-manylinux_2_34-aarch64.zip](https://github.com/NVIDIA/triton-inference-server/releases/download/v2.72.0/tritonserver_sdk-2.72.0+nv26.08-cu134-cp312-manylinux_2_34-aarch64.zip) | 405.9 MiB | `native/linux/arm64` |
| [tritonserver_sdk-2.72.0+nv26.08-cu134-cp312-manylinux_2_34-x86_64.zip](https://github.com/NVIDIA/triton-inference-server/releases/download/v2.72.0/tritonserver_sdk-2.72.0+nv26.08-cu134-cp312-manylinux_2_34-x86_64.zip) | 429.6 MiB | `native/linux/x64` |
| [v2.72.0_ubuntu2404.clients.tar.gz](https://github.com/NVIDIA/triton-inference-server/releases/download/v2.72.0/v2.72.0_ubuntu2404.clients.tar.gz) | 212.2 MiB | `native/unknown` |

## 改进这些数据

triton-inference-server 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `triton-inference-server` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/triton-inference-server.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260922.yml` · 2026-09-22T04:58:38Z._
